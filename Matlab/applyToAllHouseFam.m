function AppldAll_Vals = applyToAllHouseFam(Values, ID)
% AppldAll_Vals = applyToAllHouseFam(Values, ID)
%
% applyToAllHouseFam takes the values of an 1-D array and the corresponding ID 
...
uniqID = unique(ID);
AppldAll_Vals = NaN(length(ID),1);
for i=1:length(uniqID)
   AppldAll_Vals(ID==uniqID(i)) = sum( Values(ID==uniqID(i)) );
